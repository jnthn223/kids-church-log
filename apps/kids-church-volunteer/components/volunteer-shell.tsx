...
import { ClipboardCheck, DoorOpen, LogOut, QrCode, UserRound, WifiOff, type LucideIcon } from "lucide-react";
...
function ShellFrame({ children }: { children: ReactNode }) {
  ...
  return <div className="volunteer-app"><header className="station-header"><BrandMark /><div className="station-state"><strong>{sessionContext?.session.scheduleName || "No service selected"}</strong><span className={online ? "online" : "offline"}>{online ? "Connected" : <><WifiOff size={14} /> Offline</>}</span></div></header><main className="station-content">{children}</main><nav className="bottom-nav" aria-label="Sunday Team navigation">{navigation.map((item) => { const Icon = item.icon; const active = pathname.startsWith(item.href); return <Link key={item.href} className={active ? "active" : ""} href={item.href}><Icon size={23} /><span>{item.label}</span></Link>; })}</nav><div className="desktop-account"><span className="avatar">{member?.displayName.charAt(0) || "V"}</span><span><strong>{member?.displayName}</strong><small>Kids Church Volunteer</small></span><Button variant="ghost" aria-label="Sign out" onClick={() => void signOutUser()}><LogOut size={18} /></Button></div></div>;
}
